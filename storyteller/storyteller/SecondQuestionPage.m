...
#import "SecondQuestionPage.h"
#import "BubbleViewCell.h"
#import "Config.h"

@interface SecondQuestionPage()
...
@property (nonatomic, assign) BOOL doOffset;

@end

@implementation SecondQuestionPage

@synthesize topCell, displayTopCell, offsetHeight, doOffset, moviePlayer;
...
//    offsetHeight = 0;
    ...
    self.demoData = [[DemoModelData alloc] initWithQuestion:SecondQuestionTabIndex - 1];
    
    [self finishSendingMessageAnimated:YES];
    
    //set current tab index
    currentTab = SecondQuestionTabIndex;
    
    self.navigationController.navigationBar.topItem.title = SecondQuestionViewTitle;
    
    //define notification
    NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
    [nc addObserver:self selector:@selector(refreshResponseList:) name:NFREFRESHRESPONSELIST object:nil];
    
    passedSecondQuestionPage = YES;
    self.tabBarItem.image = [[UIImage imageNamed:@"second_question"]imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
...
- (void) refreshResponseList:(NSNotification *) note
{
    dispatch_sync(dispatch_get_main_queue(), ^{
        JSQMessage *message = [self.demoData.messages objectAtIndex:1];
        
        JSQVideoMediaItem *videoMessage = (JSQVideoMediaItem *)message.media;
        videoMessage.fileURL = [[currentTopic.questions objectAtIndex:FirstQuestionTabIndex - 1] question_response];
